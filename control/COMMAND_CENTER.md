...
## 11) coding 에이전트 실행 루프 고정 규칙
- 코드/레포/자동화 작업 요청을 받으면 "바로 하겠다", "완료본으로 보고하겠다" 같은 착수 약속만 하고 턴을 끝내지 않는다.
- 작업 요청의 첫 응답 턴 안에서 반드시 하나 이상의 실제 도구 실행을 수행한다.
  - 예: 대상 경로 확인, `git status`, 관련 파일 읽기, 테스트 실행, 파일 수정 중 하나 이상.
- 사용자가 "그냥 해", "ㄱㄱ", "진행", "나머지 진행"처럼 실행 의사를 확인하면 추가 계획 설명보다 즉시 실행을 우선한다.
- 진행이 막히면 막힌 이유를 구체적으로 보고한다.
  - 금지: "지금 바로 한다"만 반복하고 실제 실행 없이 종료.
  - 허용: 권한 부족, Gateway 오류, GitHub 인증 오류, 테스트 실패처럼 확인 가능한 원인을 짧게 보고.
- 구현 작업 완료 보고에는 항상 세 가지를 포함한다.
  1) 변경한 파일 또는 생성한 산출물
  2) 검증한 방법
  3) 남은 문제 또는 다음 행동
- 긴 Discord 세션에서 실행이 반복적으로 누락되면 새 세션으로 시작한다. 오래된 세션은 삭제하지 말고 session store에서만 분리해 transcript를 보존한다.
