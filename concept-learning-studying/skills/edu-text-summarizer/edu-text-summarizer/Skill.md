...
1. `목적`: 이 장 단계의 핵심 목적 또는 기능을 한 줄로 쓴다.
2. `안내 질문` 또는 `사용하는 질문`: 이 장에서 주로 쓰는 질문 유형을 쓴다.
...
그 아래에 이 장의 핵심 내용을 3~5문장으로 요약한다.
...
다이어그램 아래에는 아래 순서를 따른다.

1. 다이어그램 흐름을 1~2문장으로 풀어쓴다.
2. 전략군이 여러 개 제시된 장이라면, 각 전략군의 역할 차이를 2~4개 bullet로 짧게 정리한다.
3. 반드시 `### 전략별 정리` 하위 섹션을 추가한다.
4. 각 전략은 `#### 전략 1: ...`, `#### 전략 2: ...` 형식으로 쓴다.
5. 각 전략 아래에는 반드시 다음 3요소를 넣는다.
   - 공통 흐름 한 줄: `> **A → B → C**`
   - 활동 목록
   - 활동별 `ex)` 예시와 2~4줄의 전개 설명

전략별 정리는 아래 형식을 기본으로 삼는다.
...
전략 기반 장에서는 `4장 요약`처럼, 전략 설명이 단순 활동 목록이 아니라
**역할 차이 + 공통 흐름 + 활동 예시**까지 보여야 한다.
...
전략군이 분명한 장이라면, 4장 요약처럼 마지막에 `### 전략 선택 기준`을 추가할 수 있다.
이때는 `어떤 상황에서 어떤 전략이 적합한가`를 2~4개 bullet로 짧게 정리한다.
...
### 한눈에 보기의 기본 형식
`한눈에 보기`는 바로 서술문으로 시작하지 말고, 먼저 다음 두 요소를 요약해 적는다.
- `목적`: 이 장이 탐구 흐름 안에서 무엇을 하려는 단계인지
- `안내 질문` 또는 `사용하는 질문`: 사실적 질문, 개념적 질문, 호기심 촉발 질문 등

질문 유형 명칭은 사용자가 쓴 표현이나 기존 장 요약 표현을 우선 따른다.
예를 들어 4장과 9장은 `안내 질문`, 8장은 `사용하는 질문`처럼 이미 굳은 표현이 있으면 그대로 유지한다.
...
### 전략 장의 기본 형식
전략이나 활동 묶음이 제시된 장은 `개념기반탐구학습_ch04_요약.md` 형식을 기준으로 삼는다.
즉, `핵심 논지 구조` 안에서 끝내지 말고 다음을 함께 보여준다.
- 전략군별 역할 차이
- `### 전략별 정리`
- 각 전략의 공통 흐름
- 각 활동의 `ex)` 예시

특히 활동 예시는 선택이 아니라 필수다.
활동 이름만 나열하면 안 되고, 학생이 실제로 무엇을 하게 되는지 드러나야 한다.
