...
import { ArrowRight, CheckCircle2, HelpCircle, Lightbulb, RefreshCcw, RotateCcw, Trophy, XCircle } from 'lucide-react';
...
const buildOptions = (topic) => {
  const good = pick(topic.good, REQUIRED_COUNT).map((text, idx) => ({ id: `g-${topic.word}-${idx}-${text}`, text, isGood: true }));
  const bad = pick(topic.bad, REQUIRED_COUNT).map((text, idx) => ({ id: `b-${topic.word}-${idx}-${text}`, text, isGood: false }));
  return shuffle([...good, ...bad]);
};

export default function Step03({ onComplete }) {
  ...
  const handleHint = () => {
    ...
    setFeedback({ type: 'warning', msg: `힌트: "${candidate.text}"는 정답입니다. (최대 1점)` });
  };
  ...
  const resolveRound = (score, passed) => {
    setPendingResult({ word: topic.word, score, passed, hintUsed, attempts: retryUsed ? 2 : 1 });
  ...
  const restartAll = () => {
    ...
    setSelectedMap({}); setLockedMap({});
    setHintUsed(false); setRetryUsed(false);
    setRoundState('playing'); setPendingResult(null);
    setTotalScore(0); setResults([]); setIsComplete(false);
    setFeedback({ type: 'info', msg: '관련된 단어 4개를 고르세요.' });
  };
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-lg bg-white rounded-3xl shadow-xl border border-slate-200 p-8 text-center">
          ...
          <div className="bg-indigo-50 rounded-2xl p-6 mb-6">
            ...
            <div className="text-sm text-indigo-400 mt-2">정답: {solved} / {VOCAB_DATA.length}</div>
          </div>
          <button
            onClick={() => onComplete && onComplete()}
            className="w-full bg-green-500 text-white py-4 rounded-2xl font-bold hover:bg-green-600 flex items-center justify-center gap-2 mb-3 shadow-lg"
          >
            다음 단계로 →
          </button>
          <button onClick={restartAll} className="w-full bg-slate-100 text-slate-600 py-3 rounded-2xl font-bold hover:bg-slate-200 flex items-center justify-center gap-2">
  ...
  const feedbackColor = feedback.type === 'success' ? 'text-green-700 bg-green-50'
    : feedback.type === 'error' ? 'text-rose-700 bg-rose-50'
    : feedback.type === 'warning' ? 'text-amber-700 bg-amber-50'
    : 'text-slate-600 bg-slate-100';

  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between mt-12">
          <div>
            <h1 className="text-lg font-black text-slate-800">어휘의 징검다리 - Step 3</h1>
            <p className="text-xs text-slate-500">관련어 4개 고르기</p>
          </div>
      ...
      <main className="max-w-3xl mx-auto p-4 md:p-6">
        <section className="bg-white border border-slate-200 rounded-3xl p-6 mb-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => speak(topic.word)} className="px-4 py-2 rounded-xl bg-indigo-50 text-indigo-600 text-sm font-bold hover:bg-indigo-100">
              단어 듣기
            </button>
          </div>
          ...
          <div className="mt-5 text-xs text-slate-500">선택: <b>{selectedCount}</b> / {REQUIRED_COUNT}</div>
        </section>
        ...
        <section className="bg-white border border-slate-200 rounded-3xl p-5 mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4">
            <button onClick={handleHint} disabled={hintUsed || roundState !== 'playing'}
              className={`py-3 rounded-xl font-bold border flex items-center justify-center gap-2 ${
                hintUsed || roundState !== 'playing' ? 'bg-slate-100 text-slate-400 border-slate-200' : 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100'
              }`}>
              <Lightbulb size={16} /> {hintUsed ? '힌트 사용됨' : '힌트 1회'}
            </button>
            <button onClick={handleReset} disabled={roundState !== 'playing'}
              className={`py-3 rounded-xl font-bold border flex items-center justify-center gap-2 ${
                roundState !== 'playing' ? 'bg-slate-100 text-slate-400 border-slate-200' : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'
              }`}>
            ...
            <button onClick={handleCheck} disabled={roundState !== 'playing'}
              className={`py-3 rounded-xl font-bold border md:col-span-2 flex items-center justify-center gap-2 ${
                roundState !== 'playing' ? 'bg-slate-100 text-slate-400 border-slate-200' : 'bg-indigo-600 text-white border-indigo-700 hover:bg-indigo-500'
              }`}>
          ...
          {roundState === 'resolved' && pendingResult && (
            <button onClick={goNext} className="w-full mt-4 py-3 rounded-xl bg-green-600 text-white font-bold hover:bg-green-500 flex items-center justify-center gap-2">
